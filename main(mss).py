...
# --- Smoothing Variables ---
    ...
    alpha_val = 0.8  # Smoothing factor for brightness and saturation
    ...
    while True:
        ...
        if capture_counter % CAPTURE_EVERY == 0:
            ...
            if neutral_ratio > 0.35:
                ...
                prev_v = prev_v + 0.15 * (255 - prev_v)
                prev_s = prev_s + 0.15 * (0 - prev_s)
                ...
                if np.any(color_mask):
                    ...
                    dominant_hue = np.argmax(np.bincount(hue_values, minlength=180))
                    prev_hue = circular_smooth(prev_hue, dominant_hue)
                ...
                # --- SMOOTHING BRIGHTNESS & SATURATION ---
                # Alpha 0.8 provides a soft fade + instant response instead of a flicker (I found 0.5 too slow, 0.9 too fast)
